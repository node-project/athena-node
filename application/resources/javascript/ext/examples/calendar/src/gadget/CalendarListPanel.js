 ...
 */
Ext.define('Ext.calendar.gadget.CalendarListPanel', {
    extend: 'Ext.panel.Panel',
    alias: 'widget.calendarlist',
    
    requires: [
        'Ext.XTemplate',
        'Ext.calendar.gadget.CalendarListMenu'
    ],
    ...
    refresh: function(){
        ...
        var data = [], i = 0, o = null,
            CM = Ext.calendar.data.CalendarMappings,
    ...
    toggleCalendar: function(id, commit){
        var rec = this.store.findRecord(Ext.calendar.data.CalendarMappings.CalendarId.name, id);
            CM = Ext.calendar.data.CalendarMappings,
    ...
    showCalendar: function(id, commit){
        var rec = this.store.findRecord(Ext.calendar.data.CalendarMappings.CalendarId.name, id);
        if(rec.data[Ext.calendar.data.CalendarMappings.IsHidden.name] === true){
    ...
    radioCalendar: function(id){
        var i = 0, recId,
            calendarId = Ext.calendar.data.CalendarMappings.CalendarId.name,
            recs = this.store.getRange(),
            len = recs.length;
    ...
    handleColorChange: function(menu, id, colorId, origColorId){
        var rec = this.store.findRecord(Ext.calendar.data.CalendarMappings.CalendarId.name, id);
        rec.data[Ext.calendar.data.CalendarMappings.ColorId.name] = colorId;
        rec.commit();
    },
    ...
    showEventMenu : function(el, xy){
        var id = this.getCalendarId(el.parent('li')),
            rec = this.store.findRecord(Ext.calendar.data.CalendarMappings.CalendarId.name, id);
            colorId = rec.data[Ext.calendar.data.CalendarMappings.ColorId.name];
            
        if(!this.menu){
            this.menu = Ext.create('Ext.calendar.gadget.CalendarListMenu');
